...
static void log_available_video_devices() {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        if (fd >= 0) {
            ESP_LOGI(TAG, "Available video device: %s", path);
            close(fd);
            found = true;
        }
    }
    if (!found) {
        ESP_LOGE(TAG, "No video devices found in /dev/video*");
    }
}
#else
#define CAM_PRINT_FOURCC(pixelformat) (void)0;

// Force define this function if not in DEBUG mode, because we want to call it on error now
static void log_available_video_devices() {
    for (int i = 0; i < 50; i++) {
        char path[16];
        snprintf(path, sizeof(path), "/dev/video%d", i);
        int fd = open(path, O_RDONLY);
        if (fd >= 0) {
            ESP_LOGI(TAG, "found video device: %s", path); // Use LOGI to see it in standard logs
            close(fd);
        }
    }
}
...
Esp32Camera::Esp32Camera(const esp_video_init_config_t& config) {
    if (esp_video_init(&config) != ESP_OK) {
        ...
        last_error_ = "esp_video_init failed";
        return;
    }
    ...
    if (video_fd_ < 0) {
        ...
        last_error_ = "open video device failed: " + std::string(video_device_name) + " errno=" + std::to_string(errno);
#if 1 // Force enable debug logging for now
...
bool Esp32Camera::Capture() {
    ...
    if (!streaming_on_) {
        if (last_error_.empty()) {
            last_error_ = "Capture failed: streaming is not on";
        }
        ESP_LOGE(TAG, "%s", last_error_.c_str());
        return false;
    }
    if (video_fd_ < 0) {
        last_error_ = "Capture failed: video_fd is invalid (" + std::to_string(video_fd_) + ")";
        ESP_LOGE(TAG, "%s", last_error_.c_str());
        return false;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (ioctl(video_fd_, VIDIOC_DQBUF, &buf) != 0) {
            last_error_ = "VIDIOC_DQBUF failed on frame " + std::to_string(i) + ", errno=" + std::to_string(errno);
            ESP_LOGE(TAG, "%s", last_error_.c_str());
            return false;
        }
        if (i == 2) {
            ...
            if (!frame_.data) {
                last_error_ = "alloc frame copy failed: need allocate " + std::to_string(buf.bytesused) + " bytes";
                ESP_LOGE(TAG, "%s", last_error_.c_str());
                if (ioctl(video_fd_, VIDIOC_QBUF, &buf) != 0) {
