...
// Ensure WeChat message style (chat bubbles) is enabled for punctuation splitting
#ifndef CONFIG_USE_WECHAT_MESSAGE_STYLE
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (strcmp(role, "assistant") == 0) {
        ...
        for (size_t i = 0; i < str.length(); ++i) {
            ...
            if (str[i] == ',' || str[i] == '.' || str[i] == '!' || str[i] == '?') {
                ...
                current.clear();
            } else if ((unsigned char)str[i] == 0xEF && i + 2 < str.length() && 
                       (unsigned char)str[i+1] == 0xBC && 
                       ((unsigned char)str[i+2] == 0x8C || (unsigned char)str[i+2] == 0x81 || (unsigned char)str[i+2] == 0x9F)) {
                current += str[++i];
                current += str[++i];
                messages.push_back(current);
                current.clear();
            } else if ((unsigned char)str[i] == 0xE3 && i + 2 < str.length() && 
                       (unsigned char)str[i+1] == 0x80 && (unsigned char)str[i+2] == 0x82) {
                current += str[++i];
                current += str[++i];
                messages.push_back(current);
                current.clear();
    ...
    for (const auto& msg_content : messages) {
        ...
        if (strcmp(role, "system") == 0) {
            // Re-fetch child count as we might have deleted one
        ...
        lv_label_set_text(msg_text, msg_content.c_str());
        
        // Calculate actual text width
        lv_coord_t text_width = lv_txt_get_width(msg_content.c_str(), msg_content.length(), text_font, 0);
...
void LcdDisplay::ShowQrCode(const char* url) {
#if LV_USE_QRCODE
    DisplayLockGuard lock(this);
    
    // Create a full screen container for QR code that can be dismissed by click
    lv_obj_t* qr_container = lv_obj_create(lv_scr_act());
    lv_obj_set_size(qr_container, width_, height_);
    lv_obj_center(qr_container);
    lv_obj_set_style_bg_color(qr_container, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(qr_container, LV_OPA_90, 0); // Overlay
    
    // Add click event to close
    lv_obj_add_event_cb(qr_container, [](lv_event_t* e) {
        lv_obj_t* obj = lv_event_get_target(e);
        lv_obj_del(obj);
    }, LV_EVENT_CLICKED, NULL);

    int qr_size = std::min(width_, height_) * 0.8;
    ...
    // Optional: Add a label below
    lv_obj_t* label = lv_label_create(qr_container);
    lv_label_set_text(label, "Scan QR Code");
    lv_obj_align_to(label, qr, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    lv_obj_set_style_text_color(label, lv_color_white(), 0);
#else
    ESP_LOGE(TAG, "LV_USE_QRCODE is not enabled");
    ShowNotification("QR Code not supported");
#endif
}
