...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    mqtt_->OnConnected([this]() {
        ...
        ESP_LOGI(TAG, "Subscribing to command topic: xiaozhi/device/command");
        mqtt_->Subscribe("xiaozhi/device/command");
        esp_timer_stop(reconnect_timer_);
    });
    ...
    mqtt_->OnMessage([this](const std::string& topic, const std::string& payload) {
        ...
        } else if (strcmp(type->valuestring, "show_qr") == 0) {
            cJSON* url = cJSON_GetObjectItem(root, "url");
            if (cJSON_IsString(url)) {
                std::string url_str = url->valuestring;
                Application::GetInstance().Schedule([url_str]() {
                    // Board::GetInstance().GetDisplay()->ShowQrCode(url_str.c_str()); // 已禁用二维码显示
                });
            }
