...
void Es8311AudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
    ...
    i2s_std_config_t std_cfg = {
        ...
        .slot_cfg = {
            ...
            .slot_bit_width = I2S_SLOT_BIT_WIDTH_16BIT, // Force 16-bit slot to match ES8311 16-bit mode
...
void Es8311AudioCodec::SetOutputSampleRate(int rate) {
    if (rate == output_sample_rate_) {
        return;
    }
    ESP_LOGI(TAG, "Setting output sample rate to %d (old: %d)", rate, output_sample_rate_);

    AudioCodec::SetOutputSampleRate(rate);
    input_sample_rate_ = rate; // Assuming symmetric rates for duplex
    
    // 1. Reconfigure I2S Clock
    ...
    if (tx_handle_) {
        ...
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2s_channel_reconfig_std_clock(tx_handle_, &clk_cfg));
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2s_channel_enable(tx_handle_));
    }
    if (rx_handle_) {
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2s_channel_disable(rx_handle_));
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2s_channel_reconfig_std_clock(rx_handle_, &clk_cfg));
        ESP_ERROR_CHECK_WITHOUT_ABORT(i2s_channel_enable(rx_handle_));
    }

    // 2. Reconfigure Codec Device
    if (dev_) {
        esp_codec_dev_close(dev_);
        esp_codec_dev_sample_info_t fs = {
            .bits_per_sample = 16,
            .channel = 2, // STEREO: Match the 2-channel I2S stream we are sending
            .channel_mask = 0,
            .sample_rate = (uint32_t)output_sample_rate_,
            .mclk_multiple = 0,
        };
        esp_codec_dev_open(dev_, &fs);
        // Restore volume and gain
...
void Es8311AudioCodec::Start() {
    ...
    // Directly set flags and update state once to avoid partial initialization of I2S channels
    AudioCodec::EnableInput(true);
    AudioCodec::EnableOutput(true);
    UpdateDeviceState();
    
    ESP_LOGI(TAG, "Es8311AudioCodec started");
}
