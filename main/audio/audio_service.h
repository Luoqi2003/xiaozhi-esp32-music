...
// Increase the decode queue size to avoid packet drop when the network speed is faster than playback speed
#define MAX_DECODE_PACKETS_IN_QUEUE (30000 / OPUS_FRAME_DURATION_MS)
...
struct AudioServiceCallbacks {
    ...
    std::function<void(void)> on_play_finished;
...
public:
    ...
    bool IsIdle();
    // Check if there are any pending decode tasks (e.g. TTS)
    bool HasPendingDecodeTasks();
    bool IsWakeWordRunning() const { return xEventGroupGetBits(event_group_) & AS_EVENT_WAKE_WORD_RUNNING; }
    bool IsAudioProcessorRunning() const { return xEventGroupGetBits(event_group_) & AS_EVENT_AUDIO_PROCESSOR_RUNNING; }
    bool IsAfeWakeWord();

    void PlayMusic(const std::string& path);
    void PlayMusicFromMemory(const std::vector<uint8_t>& data); // New Method
